...
function useSafeTexture(url: string | undefined, fallback: THREE.Texture) {
  const [tex, setTex] = React.useState<THREE.Texture>(fallback);
  ...
  React.useEffect(() => {
    ...
    loader.load(
      ...
      (err) => {
        // 关键：失败不 throw，直接降级
        console.warn('[UserPhotoOrnament] texture load failed:', url, err);
...
}> = ({ item, mixFactor, url, signatureTexture, fallback }) => {
  const texture = useSafeTexture(url, fallback);
  return (
    <PhotoFrameMesh
      item={item}
      mixFactor={mixFactor}
