...
//       // ✅ 先更新 state（不要在这里 revoke）
//       setUserImages(urls);

//       // Reset input so selecting same files triggers change
...
const App: React.FC = () => {
  // --- 新增逻辑开始 ---
  // 获取 URL 上的参数。例如：你的网址/?to=小明
  const queryParams = new URLSearchParams(window.location.search);
  // 获取 'to' 参数，如果没有参数，默认显示 'egister'
  // decodeURIComponent 用于支持中文名字
  const recipientName = queryParams.get('to') ? decodeURIComponent(queryParams.get('to')!) : "egister";
  // --- 新增逻辑结束 ---
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      <div className={`absolute bottom-6 left-6 z-20 pointer-events-none transition-opacity duration-500 ${isSignatureOpen ? 'opacity-0' : 'opacity-100'}`}>
            <div className="text-white/20 text-[10px] uppercase tracking-widest font-luxury">
                {/* ⚠️ 这里就是修改后的动态文字 */}
                <div>specially for {recipientName}</div>
                {/* <div className="text-slate-500">Made by Southpl</div> */}
            </div>
      </div>
